...
export function registerSkillHooks(
	...
	pi.registerTool({
		...
		renderResult(result, { expanded }) {
			...
			const parsed = parseSkillBlock(content);
			if (!parsed) {
				throw new Error("load_skill result did not contain a skill block");
			}
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			// The tool result carries the skill block so the model sees the loaded
			// skill in the normal tool-result context. The renderer below displays
			// it with pi's native SkillInvocationMessageComponent.
			return {
				content: [{ type: "text" as const, text: skillBlock }],
