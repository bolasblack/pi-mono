...
describe("resolveCompatExtensionFile", () => {
	...
	test("candidates include sibling extension.mjs for flat dist/ layout", () => {
		// Actual tsdown output: all entries bundled into separate files under
		// dist/. When dist/index.mjs runs, baseDir == dist/ and the sibling
		// file is dist/extension.mjs.
		const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
		const distDir = resolve(packageRoot, "dist");
		const candidates = extensionCandidates(distDir, "/nonexistent-cwd");
		expect(candidates).toContain(resolve(distDir, "extension.mjs"));
	});

	test("candidates include parent extension.mjs for nested dist/cli/ layout", () => {
		// Nested dist layout: translate bundled under dist/cli/translate.mjs.
		// baseDir == dist/cli/, so ../extension.mjs targets dist/extension.mjs.
		const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
		const distDir = resolve(packageRoot, "dist");
		const nestedBase = resolve(distDir, "cli");
		const candidates = extensionCandidates(nestedBase, "/nonexistent-cwd");
		expect(candidates).toContain(resolve(distDir, "extension.mjs"));
	});
});
