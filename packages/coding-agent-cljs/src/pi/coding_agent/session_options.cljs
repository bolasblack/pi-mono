...
(defn build-options-map
  ...
  [{:keys [session-manager settings-manager model thinking-level scoped-models
           tools custom-tools auth-storage model-registry resource-loader]}]
  (cond-> {}
    session-manager       (assoc :sessionManager session-manager)
    settings-manager      (assoc :settingsManager settings-manager)
    ...
    (some? custom-tools)  (assoc :customTools custom-tools)
